...
Consider `relay(s32 n) { return mapper(n); }`:
...
survives — but between the prologue and epilogue there is just a single `bl`.
There's no `mr` to move the result into place because `mapper`'s return
register and `relay`'s return register are the same `r3`. The argument passes
straight through `r3` untouched as well.
...
the `bl` and a `blr` to return — never a bare `b target` that reuses the
