...
time. To see the effect, consider a simpler three-element analogue —
`twoCol_compare`, which compares two 3-byte columns `data[0]` and `data[1]`:
...
The two accumulators advance in lockstep and the schedule comes out clean — loads
in offset order, the two sum registers built up alternately:
