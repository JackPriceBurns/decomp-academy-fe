...
comparison whose boolean result is stored or returned compiles to a messier
form: the compiler still does `fcmpo`, but then materializes the 0/1 result
into a GPR, usually with `mfcr` plus `rlwinm` to extract and shift the
condition bit. If the compare result lands in a general register instead of
steering a branch, the original C stored or returned the boolean — so reach for
the plain branch shape first.) "Ordered" (`fcmpo`) vs "unordered" matters only
for NaN handling; normal C comparisons use `fcmpo`.

Now look at the target assembly for `func_8021f6f8`. Identify the constant
loaded by `lfs`, read the branch mnemonic to determine which comparison it
encodes, and figure out which return path is the early return.

## Your task

Write `func_8021f6f8` to match the target assembly. Determine the comparison
