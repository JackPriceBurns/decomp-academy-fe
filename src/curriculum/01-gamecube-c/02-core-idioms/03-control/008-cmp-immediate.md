...
the arithmetic immediates did. And the signed/unsigned split persists: a signed
`int` gives **`cmpwi`**, an unsigned operand gives **`cmplwi`**:
...
The unsigned twin (`u32 a`) would use **`cmplwi r3, 5`** on line one, and
everything else keeps the same shape. Same rule as the register compares: the
...
the two possible return values. The speculative load before the branch belongs
to the arm that exits via the branch; the fall-through `li` belongs to the
opposite arm.
