...
Now bring the subtract quirk into a chain. You've got two operations now, so two
arithmetic instructions, and the first hands its output to the second. Keep an eye
on `r0`: it carries the running result until the last instruction writes `r3`.
...
`(p + q) − r`. Get in the habit of flipping the subtrahend and minuend every time
a `subf` goes by.

Addition can be regrouped; it's associative. Subtraction can't. So the compiler
leaves your left-to-right C order intact, and the instructions line up one for
one with the operations you wrote. The reversed operands inside `subf` still
mislead.
