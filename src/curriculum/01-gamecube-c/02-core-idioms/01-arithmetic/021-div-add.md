...
that keeps no remainder, with `rA` holding the dividend and `rB` the divisor.
Nothing gets swapped the way `subf` swaps its operands, so it reads in the obvious
order.
...
So for your target, spot which argument registers reach `divw` directly, then
