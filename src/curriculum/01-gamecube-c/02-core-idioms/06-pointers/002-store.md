...
A store is a load in reverse. `stw rS, off(rA)` takes the 32-bit value in `rS`
and writes it to `rA + off`.

Watch the operand order: the source register comes first, then the address —
the opposite of a left-to-right C assignment. In `stw r4, 4(r3)`, `r4` is the
value and `r3` is the base address.
...
Now look at the target assembly for `func_80207ed8`. The displacement tells you
the element, and the two register numbers tell you which is the pointer and
which is the value.
