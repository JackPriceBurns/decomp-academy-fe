...
the same trio every time. Here is `p[1]` going up by `2`:
...
So the byte rides up into a register zero-padded, picks up its `2` where 32
bits give the sum room to breathe, and then only its lowest eight bits survive
the trip back to memory. Everything above those eight is thrown out, which is
why `255 + 1` doesn't grow into 256 but loops quietly back to `0`.
...
the sort happens here — the value just loads, shifts a little, and heads back
to memory at its original width. So `u8` and `char` agree to leave it out.
Still, `u8` is the safer habit by far: let a `char` drift into some wider
signed expression and `extsb` resurfaces unbidden, the very wrinkle `u8` spares
you.

`func_80248e18` follows the same cut — three phases in the same order. All that
...
Write `func_80248e18` to match the target. Expect exactly `lbz` / `addi` /
`stb` with no `extsb`.

<!-- solution -->
```c
void func_80248e18(u8* p) {
    p[0]++;
}
```
