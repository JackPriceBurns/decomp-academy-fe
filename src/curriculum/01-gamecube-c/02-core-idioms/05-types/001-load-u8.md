...
A register is 32 bits wide, but memory comes in bytes, halfwords, and words.
The *type* you read through decides which load instruction MWCC emits. Reading
a **`u8`** uses **`lbz`** — *load byte and zero* — which fetches one byte and
clears the upper 24 bits:
...
The "z" in `lbz` is the whole story: an unsigned byte is **zero-extended**, so
no extra instruction is needed to clean up the register. The pointer arrives in
...
*high* byte — the opposite of x86. It doesn't matter for a lone `u8`, but it
will the moment you read fields out of a struct, so keep it in mind.
